...
rszshm_mk (struct rszshm *rshm, size_t flen, const char *fname, struct rszshm_scan scan)
{
    ...
    *(rshm->hdr = (typeof (rshm->hdr)) mem) = (typeof (*rshm->hdr)) { flen, scan.len, mem };
...
rszshm_at (struct rszshm *rshm, const char *fname)
{
    ...
    if (mem == hdr.addr &&
        ...
        MAP_FAILED)
    {
        *rshm = (typeof (*rshm)) { .fd = fd,.flen = hdr.flen,.hdr = (typeof (rshm->hdr)) mem,.dat =
